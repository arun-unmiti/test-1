...
class UserMgmtView(APIView):
    def post(self, request):
        ...
        try:
            ...
            return Response({
                "success": 1,
                "message": f"New user - {email} created successfully.",
    ...
    def get(self, request):
        ...
        try:
            ...
            if user_id:
                ...
                queryset = TblUser.objects.all().select_related('role', 'adm0', 'adm1', 'adm2').prefetch_related('location_assignments', 'location_assignments__location')
                if status_:
                    status_flag = {
                        "active": True, 
                        "inactive": False
        ...
        try:
            ...
            if purpose == "edit":
                ...
                if location_id:
                    loc = LkpLocation.objects.filter(pk=int(location_id), status=True).first()
                    if loc:
                        TblUserLocation.objects.filter(user=user_obj, is_primary=True, status=True).update(is_primary=False)
                        TblUserLocation.objects.create(
                            user=user_obj,
                            location=loc,
                            location_level=loc.level,
