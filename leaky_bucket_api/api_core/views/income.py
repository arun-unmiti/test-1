...
from ..serializers import TblIncomeSerializer
...
class UtilIncome:
    def read_single(self, **kwargs):
        ...
        if item_id:
            queryobj = TblIncome.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblIncome.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under income"}, status=404)
        data = TblIncomeSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblIncome.objects.all()
        ...
        agg = queryset.aggregate(
            records=Count('id'),
            total_quantity=Sum('field_5157'),
            total_amount=Sum(F('field_5157') * F('field_5158'))
        )
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5052__icontains=search_for)
                | Q(field_5053__icontains=search_for)
                | Q(field_5055__label__icontains=search_for)
                | Q(field_5056__icontains=search_for)
                | Q(field_5057__icontains=search_for)
                | Q(field_5060__icontains=search_for)
                | Q(field_5061__icontains=search_for)
        ...
        if page and size:
            ...
            data = TblIncomeSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblIncomeSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
