...
from ..serializers import TblExpConstructionSerializer
from rest_framework.response import Response
from django.db.models import Q, Count, Sum
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger


class UtilExpConstruction:
    def read_single(self, **kwargs):
        item_id = kwargs.get("id")
        data_id = kwargs.get("data_id")
        if item_id:
            queryobj = TblExpConstruction.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblExpConstruction.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under construction expense"}, status=404)
        data = TblExpConstructionSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblExpConstruction.objects.all()
        ...
        agg = queryset.aggregate(records=Count('id'), total_quantity=Sum('field_5066'))
        return Response({"success": 1, "data": {
            "records": agg['records'] or 0,
            "total_amount": round(agg['total_quantity'] or 0, 2),
        }}, status=200)
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5062__icontains=search_for)
                | Q(field_5063__icontains=search_for)
                | Q(field_5065__label__icontains=search_for)
        ...
        if page and size:
            ...
            data = TblExpConstructionSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblExpConstructionSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
