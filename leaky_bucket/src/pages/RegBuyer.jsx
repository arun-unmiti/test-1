...
function RegBuyer({ token }) {
    ...
    useEffect(() => {
        ...
        getCore(token, 5, opts)
            .then((res) => {
                setData(res.data || []);
                setTotalCount(res.count != null ? res.count : (res.data || []).length);
            })
            .catch(() => setError("Failed to load buyer data."))
    ...
    const handleExportAll = async () => {
        ...
        try {
            ...
            if (locFilter.adm2_id) opts.adm2_id = locFilter.adm2_id;
            if (locFilter.adm3_id) opts.adm3_id = locFilter.adm3_id;
            const res = await getCore(token, 5, opts);
            exportCSV(res.data || [], `buyers_all.csv`, columns);
        } catch { /* silent */ } finally {
            setExportLoading(false);
        }
    };

    const { columns, ready: fieldsReady } = useFormFields(token, 5);
    const { summary, summaryLoading } = useCoreSummary(token, 5, locFilter);
                ...
                <div className="col-sm-12">
                    <div className="d-flex justify-content-between align-items-center flex-wrap">
                        <div className="overviewBox">
                            <h3>Buyers</h3>
                            <p>Registered buyer profiles and purchase preferences</p>
                        </div>
            ...
            <div className="row px-3 mt-3">
                <div className="col-sm-12">
                    <div className="card tbl">
                        <div className="card-body">
                            <p>Buyer Records ({loading ? "…" : totalCount})</p>
                            {error ? (
                                    ...
                                    <div className="table-responsive">
                                        <table className="table tblView">
                                            ...
                                            <tbody>
                                                {data.length === 0 ? (
                                                    <tr>
                                                        <td colSpan={columns.length + 3} className="text-center text-muted py-3">No buyers found.</td>
                                                    </tr>
                                                ) : data.map((r, i) => (
                                                    <tr key={r.id}>
