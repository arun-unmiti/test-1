from ..models import TblExpWeedMgmt
from ..serializers import TblExpWeedMgmtSerializer
from rest_framework.response import Response
from django.db.models import Q, Count, Sum
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger


class UtilExpWeedMgmt:
    def read_single(self, **kwargs):
        item_id = kwargs.get("id")
        data_id = kwargs.get("data_id")
        if item_id:
            queryobj = TblExpWeedMgmt.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblExpWeedMgmt.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under weed management expense"}, status=404)
        data = TblExpWeedMgmtSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblExpWeedMgmt.objects.all()
        ...
        agg = queryset.aggregate(records=Count('id'), total_amount=Sum('field_5126'))
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5121__icontains=search_for)
                | Q(field_5122__icontains=search_for)
                | Q(field_5124__label__icontains=search_for)
                | Q(field_5125__label__icontains=search_for)
        ...
        if page and size:
            ...
            data = TblExpWeedMgmtSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblExpWeedMgmtSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
