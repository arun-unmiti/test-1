from api_core.models import TblCoreBuyer, TblCoreFarmer, TblCoreFarm, TblCoreCrop, TblCoreSupplier
...
class WriteBuyerData:
    def __init__(self, **kwargs):
        self.row_data = kwargs.get("row_data")
        self.logged_in_user_id = kwargs.get("logged_in_user_id")
        self.model_fields = {f.name: f for f in TblCoreBuyer._meta.get_fields()}
        self.form_field_numbers = TblFormField.objects.filter(form_id=5).values_list("pk", flat=True)
        self.form_fields = [f"field_{x}" for x in TblFormField.objects.filter(form_id=5).values_list("pk", flat=True)]
    ...
    def insert(self):
        ...
        for idx, row in enumerate(self.row_data or []):
            ...
            TblCoreBuyer.objects.create(**new_data_row)
    ...
    def update(self):
        ...
        for idx, row in enumerate(self.row_data or []):
            ...
            existing_row = (
                TblCoreBuyer.objects.filter(pk=pk_).first() if pk_
                else TblCoreBuyer.objects.filter(data_id=data_id).first() if data_id
                else None
            )
            if existing_row:
                edit_data_row = {
                ...
                TblCoreBuyer.objects.filter(pk=existing_row.pk).update(**edit_data_row)
