...
from ..serializers import TblExpProcessingSerializer
from rest_framework.response import Response
from django.db.models import Q, Count, Sum
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger


class UtilExpProcessing:
    def read_single(self, **kwargs):
        item_id = kwargs.get("id")
        data_id = kwargs.get("data_id")
        if item_id:
            queryobj = TblExpProcessing.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblExpProcessing.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under processing expense"}, status=404)
        data = TblExpProcessingSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblExpProcessing.objects.all()
        ...
        agg = queryset.aggregate(records=Count('id'), total_amount=Sum('field_5150'))
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5145__icontains=search_for)
                | Q(field_5146__icontains=search_for)
                | Q(field_5148__label__icontains=search_for)
                | Q(field_5149__label__icontains=search_for)
        ...
        if page and size:
            ...
            data = TblExpProcessingSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblExpProcessingSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
