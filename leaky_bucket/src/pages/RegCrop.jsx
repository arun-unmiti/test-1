...
function RegCrop({ token }) {
    ...
    const handleExportAll = async () => {
        ...
        try {
            ...
            if (locFilter.adm2_id) opts.adm2_id = locFilter.adm2_id;
            if (locFilter.adm3_id) opts.adm3_id = locFilter.adm3_id;
            const res = await getCore(token, 3, opts);
            exportCSV(res.data || [], `crops_all.csv`, columns);
        } catch { /* silent */ } finally {
            setExportLoading(false);
        }
    };

    const { columns, ready: fieldsReady } = useFormFields(token, 3);
    const { summary, summaryLoading } = useCoreSummary(token, 3, locFilter);
                ...
                <div className="col-sm-12">
                    <div className="d-flex justify-content-between align-items-center flex-wrap">
                        <div className="overviewBox">
                            <h3>Crops</h3>
                            <p>Registered crop records per farm and farmer</p>
                        </div>
                        ...
                        <div className="row">
                            ...
                            <div className="col-sm-12 col-md-3 col-lg-3">
                                <div className="card three">
                                    <div className="card-body text-center">
                                        <h6>Total Area <span style={{ fontSize: 12, fontWeight: 400 }}>({areaUnit})</span></h6>
                                        <h4>{summaryLoading ? "—" : (summary?.crop_area ?? "—")}</h4>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div className="row px-3 mt-3">
                <div className="col-sm-12">
                    <div className="card tbl">
                        <div className="card-body">
                            <p>Crop Records ({loading ? "…" : totalCount})</p>
                            {error ? (
                                    ...
                                    <div className="table-responsive">
                                        <table className="table tblView">
                                            ...
                                            <tbody>
                                                {data.length === 0 ? (
                                                    <tr>
                                                        <td colSpan={columns.length + 3} className="text-center text-muted py-3">No crops found.</td>
                                                    </tr>
                                                ) : data.map((r, i) => (
                                                    <tr
                                                        key={r.id}
