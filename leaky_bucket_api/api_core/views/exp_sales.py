...
from ..serializers import TblExpSalesSerializer
...
class UtilExpSales:
    def read_single(self, **kwargs):
        ...
        if item_id:
            queryobj = TblExpSales.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblExpSales.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under sales expense"}, status=404)
        data = TblExpSalesSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblExpSales.objects.all()
        ...
        agg = queryset.aggregate(
            records=Count('id'),
            total_quantity=Sum('field_5058'),
            total_amount=Sum(F('field_5058') * F('field_5059'))
        )
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5151__icontains=search_for)
                | Q(field_5152__icontains=search_for)
                | Q(field_5154__label__icontains=search_for)
                | Q(field_5155__field_5049__icontains=search_for)
                | Q(field_5156__icontains=search_for)
                | Q(field_5159__icontains=search_for)
        ...
        if page and size:
            ...
            data = TblExpSalesSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblExpSalesSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
