...
from ..serializers import TblExpCropMgmtSerializer
from rest_framework.response import Response
from django.db.models import Q, Count, Sum, F
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger


class UtilExpCropMgmt:
    def read_single(self, **kwargs):
        item_id = kwargs.get("id")
        data_id = kwargs.get("data_id")
        if item_id:
            queryobj = TblExpCropMgmt.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblExpCropMgmt.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under crop management expense"}, status=404)
        data = TblExpCropMgmtSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblExpCropMgmt.objects.all()
        ...
        agg = queryset.aggregate(
            records=Count('id'),
            total_quantity=Sum('field_5113'),
            total_amount=Sum(F('field_5113') * F('field_5114'))
        )
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5109__icontains=search_for)
                | Q(field_5110__icontains=search_for)
                | Q(field_5112__icontains=search_for)
        ...
        if page and size:
            ...
            data = TblExpCropMgmtSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblExpCropMgmtSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
