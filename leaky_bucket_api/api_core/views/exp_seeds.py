from ..models import TblExpSeeds
from ..serializers import TblExpSeedsSerializer
...
class UtilExpSeeds:
    def read_single(self, **kwargs):
        ...
        if item_id:
            queryobj = TblExpSeeds.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblExpSeeds.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under seeds expense"}, status=404)
        data = TblExpSeedsSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblExpSeeds.objects.all()
        ...
        agg = queryset.aggregate(records=Count('id'), total_amount=Sum('field_5071'))
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5068__icontains=search_for)
                | Q(field_5070__label__icontains=search_for)
                | Q(crop__field_5027__crop_name__icontains=search_for)
            )
        if adm0_id: queryset = queryset.filter(field_5067__field_5016__pk=adm0_id)
        if adm1_id: queryset = queryset.filter(field_5067__field_5017__pk=adm1_id)
        if adm2_id: queryset = queryset.filter(field_5067__field_5018__pk=adm2_id)
        if adm3_id: queryset = queryset.filter(field_5067__field_5019__pk=adm3_id)
        if adm4_id: queryset = queryset.filter(field_5067__field_5020__pk=adm4_id)
        if crop_id: queryset = queryset.filter(crop__field_5027__pk__in=crop_id)
        ...
        if page and size:
            ...
            data = TblExpSeedsSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblExpSeedsSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
