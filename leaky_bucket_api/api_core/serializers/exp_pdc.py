from rest_framework import serializers
from ..models import TblExpPdc
from django.utils.timezone import datetime


class TblExpPdcSerializer(serializers.ModelSerializer):
    class Meta:
        model = TblExpPdc
        fields = "__all__"

    def to_representation(self, instance):
        is_singular = not getattr(self.parent, "many", False)
        if is_singular:
            values = {
                "id": instance.pk,
                "data_id": instance.data_id,
                "farm_id": getattr(instance.farm, "pk", None),
                "crop_id": getattr(instance.crop, "pk", None),
                "crop_lkp_id": getattr(getattr(instance.crop, "field_5027", None), "pk", None),
                "crop_name": getattr(getattr(instance.crop, "field_5027", None), "crop_name", None),
                "field_5115": instance.field_5115,
                "field_5116": instance.field_5116,
                "field_5117": instance.field_5117,
                "field_5118": getattr(instance.field_5118, "pk", None),
                "field_5119": getattr(instance.field_5119, "pk", None),
                "field_5120": instance.field_5120,
                "created_by": getattr(instance.created_by_user, "name", None),
                "latitude": instance.latitude,
                "longitude": instance.longitude,
                "created_on": datetime.strftime(instance.created_on, "%b %d, %Y %H:%M") if instance.created_on else None,
                "status": instance.status,
            }
            labels = {
                "field_5115": instance.field_5115,
                "field_5116": instance.field_5116,
                "field_5118": getattr(instance.field_5118, 'label', None),
                "field_5119": getattr(instance.field_5119, 'label', None),
            }
            return {
                "values": values,
                "labels": labels,
            }
        else:
            return {
                "id": instance.pk,
                "data_id": instance.data_id,
                "farm_id": getattr(instance.farm, "pk", None),
                "crop_id": getattr(instance.crop, "pk", None),
                "crop_lkp_id": getattr(getattr(instance.crop, "field_5027", None), "pk", None),
                "crop_name": getattr(getattr(instance.crop, "field_5027", None), "crop_name", None),
                "field_5115": instance.field_5115,
                "field_5116": instance.field_5116,
                "field_5117": instance.field_5117,
                "field_5118": getattr(instance.field_5118, 'label', None),
                "field_5119": getattr(instance.field_5119, 'label', None),
                "field_5120": instance.field_5120,
                "latitude": instance.latitude,
                "longitude": instance.longitude,
                "created_on": datetime.strftime(instance.created_on, "%b %d, %Y %H:%M") if instance.created_on else None,
                "status": instance.status,
            }
