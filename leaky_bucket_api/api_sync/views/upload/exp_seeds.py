from api_core.models import TblExpSeeds, TblCoreCrop, TblCoreFarm, TblCoreFarmer, TblCoreBuyer, TblCoreSupplier
...
class WriteExpSeedsData:
    def __init__(self, **kwargs):
        self.row_data = kwargs.get("row_data")
        self.logged_in_user_id = kwargs.get("logged_in_user_id")
        self.model_fields = {f.name: f for f in TblExpSeeds._meta.get_fields()}
        self.form_field_numbers = TblFormField.objects.filter(form_id=8).values_list("pk", flat=True)
        self.form_fields = [f"field_{x}" for x in TblFormField.objects.filter(form_id=8).values_list("pk", flat=True)]
    ...
    def insert(self):
        ...
        for idx, row in enumerate(self.row_data or []):
            ...
            farm_obj = resolve_core_ref(row, idx, "farm_id", "field_5067", TblCoreFarm, "farm")
            if isinstance(farm_obj, Response):
                return farm_obj
            new_data_row["farm_id"] = farm_obj.pk
            new_data_row["field_5067"] = farm_obj.data_id

            crop_obj = resolve_core_ref(row, idx, "crop_id", "field_5068", TblCoreCrop, "crop")
            if isinstance(crop_obj, Response):
                return crop_obj
            new_data_row["crop_id"] = crop_obj.pk
            new_data_row["field_5068"] = crop_obj.data_id
            ...
            TblExpSeeds.objects.create(**new_data_row)
    ...
    def update(self):
        ...
        for idx, row in enumerate(self.row_data or []):
            ...
            existing_row = (
                TblExpSeeds.objects.filter(pk=pk_).first() if pk_
                else TblExpSeeds.objects.filter(data_id=data_id).first() if data_id
                else None
            )
            if existing_row:
                edit_data_row = {
                ...
                edit_data_row["field_5067"] = farm_obj.data_id

                crop_obj = resolve_core_ref(row, idx, "crop_id", "field_5068", TblCoreCrop, "crop")
                if isinstance(crop_obj, Response):
                    return crop_obj
                edit_data_row["crop_id"] = crop_obj.pk
                edit_data_row["field_5068"] = crop_obj.data_id
                ...
                TblExpSeeds.objects.filter(pk=existing_row.pk).update(**edit_data_row)
