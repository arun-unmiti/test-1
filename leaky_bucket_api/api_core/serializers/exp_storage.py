from rest_framework import serializers
from ..models import TblExpStorage
from django.utils.timezone import datetime


class TblExpStorageSerializer(serializers.ModelSerializer):
    class Meta:
        model = TblExpStorage
        fields = "__all__"

    def to_representation(self, instance):
        is_singular = not getattr(self.parent, "many", False)
        if is_singular:
            values = {
                "id": instance.pk,
                "data_id": instance.data_id,
                "farm_id": getattr(instance.farm, "pk", None),
                "crop_id": getattr(instance.crop, "pk", None),
                "crop_lkp_id": getattr(getattr(instance.crop, "field_5027", None), "pk", None),
                "crop_name": getattr(getattr(instance.crop, "field_5027", None), "crop_name", None),
                "field_5139": instance.field_5139,
                "field_5140": instance.field_5140,
                "field_5141": instance.field_5141,
                "field_5142": getattr(instance.field_5142, "pk", None),
                "field_5143": getattr(instance.field_5143, "pk", None),
                "field_5144": instance.field_5144,
                "created_by": getattr(instance.created_by_user, "name", None),
                "latitude": instance.latitude,
                "longitude": instance.longitude,
                "created_on": datetime.strftime(instance.created_on, "%b %d, %Y %H:%M") if instance.created_on else None,
                "status": instance.status,
            }
            labels = {
                "field_5139": instance.field_5139,
                "field_5140": instance.field_5140,
                "field_5142": getattr(instance.field_5142, 'label', None),
                "field_5143": getattr(instance.field_5143, 'label', None),
            }
            return {
                "values": values,
                "labels": labels,
            }
        else:
            return {
                "id": instance.pk,
                "data_id": instance.data_id,
                "farm_id": getattr(instance.farm, "pk", None),
                "crop_id": getattr(instance.crop, "pk", None),
                "crop_lkp_id": getattr(getattr(instance.crop, "field_5027", None), "pk", None),
                "crop_name": getattr(getattr(instance.crop, "field_5027", None), "crop_name", None),
                "field_5139": instance.field_5139,
                "field_5140": instance.field_5140,
                "field_5141": instance.field_5141,
                "field_5142": getattr(instance.field_5142, 'label', None),
                "field_5143": getattr(instance.field_5143, 'label', None),
                "field_5144": instance.field_5144,
                "latitude": instance.latitude,
                "longitude": instance.longitude,
                "created_on": datetime.strftime(instance.created_on, "%b %d, %Y %H:%M") if instance.created_on else None,
                "status": instance.status,
            }
