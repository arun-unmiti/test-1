...
class WriteCropData:
    def __init__(self, **kwargs):
        ...
        self.model_fields = {f.name: f for f in TblCoreCrop._meta.get_fields()}
        self.form_field_numbers = TblFormField.objects.filter(form_id=3).values_list("pk", flat=True)
        self.form_fields = [f"field_{x}" for x in TblFormField.objects.filter(form_id=3).values_list("pk", flat=True)]
    ...
    def insert(self):
        ...
        for idx, row in enumerate(self.row_data or []):
            ...
            farmer_obj = resolve_core_ref(row, idx, "farmer_id", "field_5024", TblCoreFarmer, "farmer")
            if isinstance(farmer_obj, Response):
                return farmer_obj
            new_data_row["farmer_id"] = farmer_obj.pk
            new_data_row["field_5024"] = farmer_obj.data_id

            farm_obj = resolve_core_ref(row, idx, "farm_id", "field_5025", TblCoreFarm, "farm")
            if isinstance(farm_obj, Response):
                return farm_obj
            new_data_row["farm_id"] = farm_obj.pk
            new_data_row["field_5025"] = farm_obj.data_id
            ...
            TblCoreCrop.objects.create(**new_data_row)
    ...
    def update(self):
        ...
        for idx, row in enumerate(self.row_data or []):
            ...
            existing_row = (
                TblCoreCrop.objects.filter(pk=pk_).first() if pk_
                else TblCoreCrop.objects.filter(data_id=data_id).first() if data_id
                else None
            )
            if existing_row:
                edit_data_row = {
                ...
                if isinstance(farmer_obj, Response):
                    return farmer_obj
                edit_data_row["farmer_id"] = farmer_obj.pk
                edit_data_row["field_5024"] = farmer_obj.data_id

                farm_obj = resolve_core_ref(row, idx, "farm_id", "field_5025", TblCoreFarm, "farm")
                if isinstance(farm_obj, Response):
                    return farm_obj
                edit_data_row["farm_id"] = farm_obj.pk
                edit_data_row["field_5025"] = farm_obj.data_id
                ...
                TblCoreCrop.objects.filter(pk=existing_row.pk).update(**edit_data_row)
