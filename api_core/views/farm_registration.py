...
class UtilCoreFarm:
    ...
    def read_list(self, **kwargs):
        ...
        queryset = TblCoreFarm.objects.filter(loc_filter_criteria)
        # if search_for:
        #     search_criteria = (
        #         Q(field_5001__name__icontains=search_for)
        #         | Q(field_5002__name__icontains=search_for)
        #         | Q(field_5003__name__icontains=search_for)
        #         | Q(field_5004__name__icontains=search_for)
        #         | Q(field_5005__name__icontains=search_for)
        #         # | more filterable fields here as required
        #     )
        ...
        if page and size:
            ...
            data = TblCoreFarmSerializer(page_queryset, many=True).data
            return Response({
                "success": 1,
