...
function RegSupplier({ token }) {
    ...
    const handleExportAll = async () => {
        ...
        try {
            ...
            if (locFilter.adm2_id) opts.adm2_id = locFilter.adm2_id;
            if (locFilter.adm3_id) opts.adm3_id = locFilter.adm3_id;
            const res = await getCore(token, 4, opts);
            exportCSV(res.data || [], `suppliers_all.csv`, columns);
        } catch { /* silent */ } finally {
            setExportLoading(false);
        }
    };

    const { columns, ready: fieldsReady } = useFormFields(token, 4);
    const { summary, summaryLoading } = useCoreSummary(token, 4, locFilter);
                ...
                <div className="col-sm-12">
                    <div className="d-flex justify-content-between align-items-center flex-wrap">
                        <div className="overviewBox">
                            <h3>Suppliers</h3>
                            <p>Registered supplier profiles and supply materials</p>
                        </div>
            ...
            <div className="row px-3 mt-3">
                <div className="col-sm-12">
                    <div className="card tbl">
                        <div className="card-body">
                            <p>Supplier Records ({loading ? "…" : totalCount})</p>
                            {error ? (
                                    ...
                                    <div className="table-responsive">
                                        <table className="table tblView">
                                            ...
                                            <tbody>
                                                {data.length === 0 ? (
                                                    <tr>
                                                        <td colSpan={columns.length + 3} className="text-center text-muted py-3">No suppliers found.</td>
                                                    </tr>
                                                ) : data.map((r, i) => (
                                                    <tr key={r.id}>
