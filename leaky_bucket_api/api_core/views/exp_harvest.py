...
from ..serializers import TblExpHarvestSerializer
from rest_framework.response import Response
from django.db.models import Q, Count, Sum
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger


class UtilExpHarvest:
    def read_single(self, **kwargs):
        item_id = kwargs.get("id")
        data_id = kwargs.get("data_id")
        if item_id:
            queryobj = TblExpHarvest.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblExpHarvest.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under harvest expense"}, status=404)
        data = TblExpHarvestSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblExpHarvest.objects.all()
        ...
        agg = queryset.aggregate(records=Count('id'), total_amount=Sum('field_5132'))
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5127__icontains=search_for)
                | Q(field_5128__icontains=search_for)
                | Q(field_5130__label__icontains=search_for)
                | Q(field_5131__label__icontains=search_for)
        ...
        if page and size:
            ...
            data = TblExpHarvestSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblExpHarvestSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
