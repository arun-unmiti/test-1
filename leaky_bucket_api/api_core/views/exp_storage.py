from ..models import TblExpStorage
from ..serializers import TblExpStorageSerializer
from rest_framework.response import Response
from django.db.models import Q, Count, Sum
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger


class UtilExpStorage:
    def read_single(self, **kwargs):
        item_id = kwargs.get("id")
        data_id = kwargs.get("data_id")
        if item_id:
            queryobj = TblExpStorage.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblExpStorage.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under storage expense"}, status=404)
        data = TblExpStorageSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblExpStorage.objects.all()
        ...
        agg = queryset.aggregate(records=Count('id'), total_amount=Sum('field_5144'))
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5139__icontains=search_for)
                | Q(field_5140__icontains=search_for)
                | Q(field_5142__label__icontains=search_for)
                | Q(field_5143__label__icontains=search_for)
        ...
        if page and size:
            ...
            data = TblExpStorageSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblExpStorageSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
