...
from ..serializers import TblCoreCoopGroupSerializer
...
class UtilCoreCoopGroup:
    def read_single(self, **kwargs):
        # coop_group_id = kwargs.get("coop_group_id")
        data_id = kwargs.get("data_id")
        queryobj = TblCoreCoopGroup.objects.filter(data_id=data_id).first()
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under cooperative/group registration"}, status=404)
        data = TblCoreCoopGroupSerializer(queryobj, many=False).data
    ...
    def read_list(self, **kwargs):
        ...
        queryset = TblCoreCoopGroup.objects.filter(loc_filter_criteria)
        # if search_for:
        #     search_criteria = (
        ...
        if page and size:
            ...
            data = TblCoreCoopGroupSerializer(page_queryset, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblCoreCoopGroupSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)