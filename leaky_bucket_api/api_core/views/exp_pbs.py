...
from ..serializers import TblExpPbsSerializer
...
class UtilExpPbs:
    def read_single(self, **kwargs):
        ...
        if item_id:
            queryobj = TblExpPbs.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblExpPbs.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under pbs expense"}, status=404)
        data = TblExpPbsSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblExpPbs.objects.all()
        ...
        agg = queryset.aggregate(records=Count('id'), total_amount=Sum('field_5138'))
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5133__icontains=search_for)
                | Q(field_5134__icontains=search_for)
                | Q(field_5136__label__icontains=search_for)
                | Q(field_5137__label__icontains=search_for)
        ...
        if page and size:
            ...
            data = TblExpPbsSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblExpPbsSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
