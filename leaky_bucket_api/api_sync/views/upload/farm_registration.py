...
class WriteFarmData:
    ...
    def insert(self):
        ...
        for idx, row in enumerate(self.row_data or []):
            ...
            farmer_obj = resolve_core_ref(row, idx, "farmer_id", "field_5014", TblCoreFarmer, "farmer")
            if isinstance(farmer_obj, Response):
                return farmer_obj
            new_data_row["farmer_id"] = farmer_obj.pk
            new_data_row["field_5014"] = farmer_obj.data_id
    ...
    def update(self):
        ...
        for idx, row in enumerate(self.row_data or []):
            ...
            existing_row = (
                TblCoreFarm.objects.filter(pk=pk_).first() if pk_
                else TblCoreFarm.objects.filter(data_id=data_id).first() if data_id
                else None
            )
            if existing_row:
                edit_data_row = {
                ...
                edit_data_row["field_5014"] = farmer_obj.data_id
                ...
                TblCoreFarm.objects.filter(pk=existing_row.pk).update(**edit_data_row)
