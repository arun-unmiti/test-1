from ..models import TblExpPdc
from ..serializers import TblExpPdcSerializer
...
class UtilExpPdc:
    def read_single(self, **kwargs):
        ...
        if item_id:
            queryobj = TblExpPdc.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblExpPdc.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under pdc expense"}, status=404)
        data = TblExpPdcSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblExpPdc.objects.all()
        ...
        agg = queryset.aggregate(records=Count('id'), total_amount=Sum('field_5120'))
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5115__icontains=search_for)
                | Q(field_5116__icontains=search_for)
                | Q(field_5118__label__icontains=search_for)
                | Q(field_5119__label__icontains=search_for)
        ...
        if page and size:
            ...
            data = TblExpPdcSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblExpPdcSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
