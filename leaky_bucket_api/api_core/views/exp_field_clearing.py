from ..models import TblExpFieldClearing
from ..serializers import TblExpFieldClearingSerializer
from rest_framework.response import Response
from django.db.models import Q, Count, Sum
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger


class UtilExpFieldClearing:
    def read_single(self, **kwargs):
        item_id = kwargs.get("id")
        data_id = kwargs.get("data_id")
        if item_id:
            queryobj = TblExpFieldClearing.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblExpFieldClearing.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under field clearing expense"}, status=404)
        data = TblExpFieldClearingSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblExpFieldClearing.objects.all()
        ...
        agg = queryset.aggregate(records=Count('id'), total_amount=Sum('field_5076'))
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5072__icontains=search_for)
                | Q(field_5073__icontains=search_for)
                | Q(field_5075__label__icontains=search_for)
        ...
        if page and size:
            ...
            data = TblExpFieldClearingSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblExpFieldClearingSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
