...
from ..serializers import TblExpIrrigationSerializer
from rest_framework.response import Response
from django.db.models import Q, Count, Sum
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger


class UtilExpIrrigation:
    def read_single(self, **kwargs):
        item_id = kwargs.get("id")
        data_id = kwargs.get("data_id")
        if item_id:
            queryobj = TblExpIrrigation.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblExpIrrigation.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under irrigation expense"}, status=404)
        data = TblExpIrrigationSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblExpIrrigation.objects.all()
        ...
        agg = queryset.aggregate(records=Count('id'), total_amount=Sum('field_5108'))
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5102__icontains=search_for)
                | Q(field_5103__icontains=search_for)
                | Q(field_5105__label__icontains=search_for)
                | Q(field_5106__label__icontains=search_for)
                | Q(field_5107__label__icontains=search_for)
        ...
        if page and size:
            ...
            data = TblExpIrrigationSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblExpIrrigationSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
