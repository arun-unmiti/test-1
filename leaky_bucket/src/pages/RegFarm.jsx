...
function RegFarm({ token }) {
    ...
    const handleExportAll = async () => {
        ...
        try {
            ...
            if (locFilter.adm2_id) opts.adm2_id = locFilter.adm2_id;
            if (locFilter.adm3_id) opts.adm3_id = locFilter.adm3_id;
            const res = await getCore(token, 2, opts);
            exportCSV(res.data || [], `farms_all.csv`, columns);
        } catch { /* silent */ } finally {
            setExportLoading(false);
        }
    };

    const { columns, ready: fieldsReady } = useFormFields(token, 2);
    const { summary, summaryLoading } = useCoreSummary(token, 2, locFilter);
                            ...
                            <div className="col-sm-12 col-md-3 col-lg-3">
                                <div className="card three">
                                    <div className="card-body text-center">
                                        ...
                                        <h4>{summaryLoading ? "—" : (summary?.covered_area ?? "—")}</h4>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div className="row px-3 mt-3">
                <div className="col-sm-12">
                    <div className="card tbl">
                        <div className="card-body">
                            <p>Farm Records ({loading ? "…" : totalCount})</p>
                            {error ? (
                                    ...
                                    <div className="table-responsive">
                                        <table className="table tblView">
                                            ...
                                            <tbody>
                                                {data.length === 0 ? (
                                                    <tr>
                                                        <td colSpan={columns.length + 3} className="text-center text-muted py-3">No farms found.</td>
                                                    </tr>
                                                ) : data.map((r, i) => (
                                                    <tr
                                                        key={r.id}
