from ..models import TblExpPlanting
from ..serializers import TblExpPlantingSerializer
from rest_framework.response import Response
from django.db.models import Q, Count, Sum
from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger


class UtilExpPlanting:
    def read_single(self, **kwargs):
        item_id = kwargs.get("id")
        data_id = kwargs.get("data_id")
        if item_id:
            queryobj = TblExpPlanting.objects.filter(pk=item_id).first()
        elif data_id:
            queryobj = TblExpPlanting.objects.filter(data_id=data_id).first()
        else:
            queryobj = None
        if not queryobj:
            return Response({"success": 0, "message": "The requested record is not found under planting expense"}, status=404)
        data = TblExpPlantingSerializer(queryobj, many=False).data
    ...
    def read_summary(self, **kwargs):
        ...
        queryset = TblExpPlanting.objects.all()
        ...
        agg = queryset.aggregate(records=Count('id'), total_amount=Sum('field_5097'))
    ...
    def read_list(self, **kwargs):
        ...
        if search_for:
            queryset = queryset.filter(
                Q(field_5092__icontains=search_for)
                | Q(field_5093__icontains=search_for)
                | Q(field_5095__label__icontains=search_for)
                | Q(field_5096__label__icontains=search_for)
        ...
        if page and size:
            ...
            data = TblExpPlantingSerializer(page_obj.object_list, many=True).data
            return Response({
                "success": 1,
            ...
            data = TblExpPlantingSerializer(queryset, many=True).data
            return Response({"success": 1, "data": data}, status=200)
