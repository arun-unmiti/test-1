from api_core.models import TblCoreAnimal, TblCoreHerd, TblCoreFarm
...
class WriteAnimalData:
    ...
    def insert(self):
        ...
        for row in self.row_data:
            ...
            farm_data_id = TblCoreFarm.objects.filter(pk=row.get("field_5032")).first().data_id
            herd_data_id = TblCoreHerd.objects.filter(pk=row.get("field_5033")).first().data_id
            new_data_row = {
                "data_id": generate_data_id(self.logged_in_user_id),
                "farm_data_id": farm_data_id, 
                "herd_data_id": herd_data_id,
            ...
            TblCoreAnimal.objects.create(**new_data_row)
    ...
    def update(self):
        ...
        for row in self.row_data:
            ...
            existing_row = TblCoreAnimal.objects.filter(pk=pk_)
            if existing_row:
                edit_data_row = {
